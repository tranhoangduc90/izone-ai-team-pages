(function () {
  ...
  if (['complete', 'listening-only', 'writing-prep', 'writing'].includes(demoMode)) {
    if (!window.K56_SUBSTITUTE_TEST_2_CONTENT) {
    ...
    Promise.resolve()
      .then(() => loadScript('../substitute-test-2-k56-shared/app.js?v=20260918-writing-async'))
  ...
  root.innerHTML = `
    <header class="topbar cbt-bootstrap-topbar">
      <h1>Substitute Test 2 · Khóa 56</h1>
    </header>
  ...
  async function downloadLocalDemoAudio() {
    ...
    try {
      const source = window.K56_SUBSTITUTE_TEST_2_CONTENT?.audio?.src;
  ...
  async function enterExam(started, audioElement = null) {
    ...
    previewAudio.remove();
    revokePreview();
    await loadScript('../substitute-test-2-k56-shared/app.js?v=20260918-writing-async');
