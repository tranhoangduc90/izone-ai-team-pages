(function () {
  ...
  if (['complete', 'listening-only', 'writing-prep', 'writing'].includes(demoMode)) {
    if (!window.K56_SUBSTITUTE_TEST_CONTENT) {
    ...
    Promise.resolve()
      .then(() => loadScript('../substitute-k56-shared/app.js'))
  ...
  if (localDemo && query.get('reset') === '1') {
    for (const storage of [sessionStorage, localStorage]) {
      try {
        for (const namespace of ['RETAKE-LOBBY', 'K56A', 'K56B', 'K56C']) {
  ...
  root.innerHTML = `
    <header class="topbar cbt-bootstrap-topbar">
      <h1>Substitute Test 1 · Khóa 56</h1>
    </header>
  ...
  async function downloadLocalDemoAudio() {
    ...
    try {
      const source = window.K56_SUBSTITUTE_TEST_CONTENT?.audio?.src;
    ...
    try {
      if (localDemo) {
        if (state.attemptToken) {
          ...
          await enterExam({
            content: window.K56_SUBSTITUTE_TEST_CONTENT,
            serverNow,
  ...
  async function enterExam(started, audioElement = null) {
    ...
    previewAudio.remove();
    revokePreview();
    await loadScript('../substitute-k56-shared/app.js');
