(function () {
  ...
  if (['complete', 'listening-only', 'writing-prep', 'writing', 'reading', 'listening'].includes(demoMode)) {
    ...
    Promise.resolve()
      .then(() => loadScript('../k56-mini-shared/app.js?v=20260914-mini-writing-revision-v1-student-feedback-v1-html-v2-teacher-parity-v1'))
    ...
    <main class="page-shell cbt-bootstrap-shell">
      <section class="panel cbt-bootstrap-card" aria-labelledby="securePrepTitle">
        <header class="cbt-lobby-header">
          ...
          <p>Đề thi đang được khóa. Hãy xác nhận họ tên, tải đủ audio và nghe thử trước khi bắt đầu.</p>
        </header>
  ...
  function saveState(patch = {}) {
    ...
    if (localDemo) {
      history.replaceState({ ...(history.state || {}), k56MiniDemoState: state }, '', window.location.href);
    }
  }
  ...
  function setStartAvailability() {
    ...
    elements.bootstrapStartStatus.textContent = resumed
      ...
      : 'Sau khi bấm, audio phát từ đầu và đề mới được mở.';
  }
  ...
  async function enterExam(started, audioElement = null) {
    ...
    previewAudio.remove();
    revokePreview();
    await loadScript('../k56-mini-shared/app.js?v=20260914-mini-writing-revision-v1-student-feedback-v1-html-v2-teacher-parity-v1');
