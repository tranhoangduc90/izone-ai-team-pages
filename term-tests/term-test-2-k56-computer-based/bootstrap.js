(function () {
  ...
  if (['complete', 'listening-only', 'writing-prep', 'writing', 'reading', 'listening'].includes(demoMode)) {
    ...
    Promise.resolve()
      .then(() => loadScript('../k56-test2-shared/app.js?v=20260913-writing-revision-v1-task1-result-v2-student-feedback-v1-html-v2-teacher-parity-v1'))
  ...
  function readState() {
    ...
    if (localDemo && history.state?.k56Test2DemoState) return { ...history.state.k56Test2DemoState };
    return {};
  }
  ...
  function saveState(patch = {}) {
    ...
    if (localDemo) {
      history.replaceState({ ...(history.state || {}), k56Test2DemoState: state }, '', window.location.href);
    }
  }
  ...
  async function enterExam(started, audioElement = null) {
    ...
    previewAudio.remove();
    revokePreview();
    await loadScript('../k56-test2-shared/app.js?v=20260913-writing-revision-v1-task1-result-v2-student-feedback-v1-html-v2-teacher-parity-v1');
