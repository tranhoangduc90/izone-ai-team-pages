(function () {
  ...
  function collectFields(grid) {
    ...
    grid.querySelectorAll('.question').forEach(question => {
      question.querySelectorAll('[data-number]').forEach(field => {
  ...
  function formatMinutes(seconds) {
    return String(Math.ceil(Math.max(0, Number(seconds) || 0) / 60));
  }
  ...
  function syncRadioField(field, card) {
    ...
    const radios = [...card.querySelectorAll('[data-choice-value]')].map(label => {
      label.classList.add('cbt-choice');
      label.querySelector(':scope > strong')?.classList.add('cbt-choice-letter');
  ...
  function attachAnswerFields(skill, sectionNode, sectionIndex, fields, items) {
    ...
    sectionNode.querySelectorAll('[data-control="pair"]').forEach(card => {
      const keys = (card.dataset.answerKeys || '').split(',').filter(Boolean);
      const pairFields = keys.map(key => fields.get(key)).filter(Boolean);
      if (pairFields.length !== keys.length) return;

      keys.forEach((key, index) => {
        const slot = card.querySelector('[data-answer-slot="' + key + '"]');
        if (!slot) return;
        const field = pairFields[index];
        field.classList.add('cbt-answer-input', 'cbt-pair-answer');
        slot.append(field);
        processedSlots.add(slot);
      });

      const number = String(card.dataset.questionNumber || keys[0]);
  ...
  function createQuestionNav(skill, source) {
    ...
    function itemFields(item) {
      return item.fields || [item.field];
    }

    function itemAnswered(item) {
      return itemFields(item).every(field => Boolean(field.value.trim()));
    }

    function updateItem(item) {
      const answered = itemAnswered(item);
      ...
      item.target.classList.toggle('is-answered', targetItems.some(itemAnswered));
    ...
    function focusItem(item) {
      ...
      window.requestAnimationFrame(() => {
        ...
        const focusTarget = item.radios.find(radio => radio.checked)
          || item.radios[0]
          || itemFields(item).find(field => !field.value.trim())
          || item.field;
  ...
  function setupBufferedAudio(listeningForm) {
    ...
    playbackProgress.setAttribute('aria-label', 'Tiến độ audio Listening');
    ...
    function updateExamStatus(prefix) {
      ...
      playbackProgress.setAttribute('aria-valuetext', formatTime(current) + ' / ' + (duration ? formatTime(duration) : contentConfig.audio.durationLabel));
    ...
    function reportAudioProgress(playbackState, keepalive = false) {
      ...
      if (!examSessionToken || !examStarted || !['term-test-1-k56', 'term-test-2-k56', 'mini-test-k56', 'substitute-test-1-k67'].includes(testConfig.slug)) return Promise.resolve(null);
      ...
      const send = async () => {
        ...
        if (payload.listeningDeadlineAt) window.dispatchEvent(new CustomEvent('term-test:listening-timing-updated', { detail: payload }));
        return payload;
      };
    ...
    async function recoverInterruptedAudio(trigger, force = false) {
      ...
      try {
        if (!audio.paused) { allowPause = true; audio.pause(); allowPause = false; }
        audio.currentTime = Math.min(Math.max(0, heardSeconds), Math.max(0, (audio.duration || heardSeconds + 1) - 0.05));
        await audio.play();
        resumeButton.hidden = true;
        examRetryButton.hidden = true;
      ...
      } finally { allowPause = false; recoveryInFlight = false; }
    }
    ...
    audio.addEventListener('timeupdate', () => {
      ...
      if (audio.currentTime > lastObservedAudioTime + 0.05) { lastObservedAudioTime = audio.currentTime; lastAudioAdvanceAt = performance.now(); }
    ...
    audio.addEventListener('waiting', () => window.setTimeout(() => {
    ...
    window.addEventListener('pagehide', () => {
      allowPause = true;
      window.clearInterval(audioWatchdog);
      if (examStarted) { rememberActualHeardPosition(true); reportAudioProgress('pagehide', true).catch(() => null); }
      downloadController?.abort();
      revokeAudioUrl();
    });
    ...
    audioWatchdog = window.setInterval(() => {
      ...
      if (current > lastObservedAudioTime + 0.05) { lastObservedAudioTime = current; lastAudioAdvanceAt = performance.now(); return; }
      if (audio.paused) recoverInterruptedAudio('pause');
      else if (performance.now() - lastAudioAdvanceAt >= 1800) recoverInterruptedAudio(audio.readyState < 3 ? 'waiting' : 'stalled');
    }, 500);
  }
  ...
  // Việc chính: dùng deadline Reading do máy chủ cấp, cảnh báo 10 phút cuối và tự gửi khi hết giờ.
  ...
  function setupReadingTimer(readingForm) {
    ...
    const totalMinutes = Number(testConfig.reading.durationMinutes) || 40;
    ...
    clockValue.textContent = String(totalMinutes);
    ...
    function renderTimer() {
      ...
      if (remainingSeconds !== lastRenderedSecond) {
        ...
        clockValue.textContent = formatMinutes(remainingSeconds);
        clock.dataset.seconds = String(remainingSeconds);
        clock.classList.toggle('is-warning', remainingSeconds <= 600 && remainingSeconds > 0);
        clock.classList.toggle('is-expired', remainingSeconds === 0);
        clock.setAttribute('aria-label', remainingSeconds > 0
          ? 'Còn ' + formatMinutes(remainingSeconds) + ' trên ' + totalMinutes + ' phút làm bài Reading'
          : 'Đã hết ' + totalMinutes + ' phút làm bài Reading; hệ thống đang tự nộp bài');
      }
  ...
  // Việc chính: dùng deadline Writing do máy chủ cấp, cảnh báo 10 phút cuối và gọi đúng luồng nộp Writing khi hết giờ.
  ...
  function setupWritingTimer() {
    ...
    const totalMinutes = Number(testConfig.writing?.durationMinutes ?? contentConfig.writing?.totalMinutes ?? 30);
    const planningMinutes = Number(testConfig.writing?.planningMinutes ?? contentConfig.writing?.planningMinutes ?? 0);
  ...
  if (topbarTitle) topbarTitle.textContent = 'Substitute Test 1 · Khóa 67';
