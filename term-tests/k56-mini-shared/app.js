(function () {
  ...
  const writingMarkup = writingConfig ? `
      <section class="panel transition-card writing-prep-card" id="writingPrepView" hidden>
        ...
        <p>Kết quả Listening và Reading đang được giữ kín. Khi sẵn sàng, hãy bắt đầu Writing · Paragraph.</p>
        <ul class="writing-prep-list">
          <li>Tổng thời gian: ${writingConfig.totalMinutes} phút, gồm ${writingConfig.planningMinutes} phút lập dàn ý và ${writingConfig.totalMinutes - writingConfig.planningMinutes} phút viết bài.</li>
          <li>Viết đoạn văn ít nhất 100 từ.</li>
      ...
      <form class="panel writing-exam-view" id="writingView" hidden>
        <header class="writing-exam-header">
          <div>
            <p class="eyebrow">Phần 3 · Writing</p>
            <h2>Writing · Paragraph</h2>
          </div>
        ...
        <footer class="writing-task-footer" ${writingConfig.tasks?.length === 1 ? 'hidden' : ''}>
          ...
          <span id="writingTaskPosition">Paragraph · 1 / 1</span>
  ...
  root.innerHTML = `
    <header class="topbar">
      <p class="eyebrow">IZONE · IELTS 5–6</p>
  ...
  function setupWritingExam() {
    ...
    function makeWritingPanel(task) {
      ...
      outlineHint.textContent = '3 phút';
      ...
      outlineEditor.setAttribute('aria-label', 'Dàn ý Writing · Paragraph');
  ...
  function openWritingFeedback(taskResult) {
    ...
    title.textContent = `${task.label} · Điểm đoạn văn ${formatBand(taskResult.taskScore)}`;
    const meta = document.createElement('p');
    meta.textContent = `${Number(taskResult.wordCount || countWords(essayValue))} từ · Rubric đoạn văn · không quy đổi IELTS band`;
    ...
    for (const criterion of Array.from(taskResult.criteria || [])) {
      ...
      value.textContent = `Điểm đoạn văn ${formatBand(criterion.bandScore)}`;
    ...
    for (const criterion of Array.from(taskResult.criteria || [])) {
      ...
      criterionScore.textContent = `Điểm đoạn văn ${formatBand(criterion.bandScore)}`;
  ...
  function renderWritingSubmission() {
    ...
    if (grading?.ready) {
      ...
      refresh.hidden = serverGradingMode;
  ...
  elements.listeningView.addEventListener('submit', async event => {
    ...
    try {
      if (demoMode === 'exam') {
        ...
        showNotice(serverGradingMode
          ? `Backend test đã chấm Listening: ${state.testGrades.listening.correct}/10 câu đúng.`
  ...
  elements.readingView.addEventListener('submit', async event => {
    ...
    try {
      if (demoMode === 'exam') {
        ...
        showNotice(serverGradingMode
          ? `Backend test đã chấm Reading: ${state.testGrades.reading.correct}/13 câu đúng. Kết quả sẽ mở sau Writing.`
  ...
  if (writingConfig) {
    elements.startWriting.addEventListener('click', async () => {
      ...
      if (demoMode === 'exam') {
        state.writingDeadlineAt = new Date(Date.now() + (Number(testConfig.writing?.durationMinutes) || 30) * 60_000).toISOString();
        state.serverTimeOffsetMs = 0;
      }
      saveSession();
  ...
  function buildDemoPayload(mode) {
    const listening = serverGradingMode
      ? state.testGrades.listening || makeDemoSection('Listening', 0, 10)
      : makeDemoSection('Listening', 0, 10);
    const reading = mode === 'complete'
      ? serverGradingMode
        ? state.testGrades.reading || makeDemoSection('Reading', 0, 13)
        : makeDemoSection('Reading', 0, 13)
      : null;
    ...
    const demoWriting = mode === 'complete' && serverGradingMode ? {
      ...
      grading: {
        ...
        ready: false,
        taskStates: { task1: 'pending_test_integration' }
      }
    } : mode === 'complete' ? {
      task1: state.drafts.writing.task1,
      started: true,
  ...
  async function initialize() {
    ...
    if (demoMode) {
      ...
      if (demoMode === 'complete' && writingConfig) {
        state.writingSubmitted = true;
        state.drafts.writing.task1 = 'This is a sample Paragraph response. The live page preserves the student essay exactly as typed and provides a separate copy button for each task.';
      }
      if (demoMode === 'exam') {
        const bootstrappedIdentity = Boolean(window.TERM_TEST_BOOTSTRAP && state.studentRef);
        if (bootstrappedIdentity) {
          state.studentName = state.studentName || 'Học viên Demo';
          state.className = classCode || 'CODEXDEMO56';
