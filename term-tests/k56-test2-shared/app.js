(function () {
  ...
  const requestedDemo = ['127.0.0.1', 'localhost'].includes(location.hostname) ? (query.get('demo') || '') : '';
  ...
  const writingMarkup = writingConfig ? `
      <section class="panel transition-card writing-prep-card" id="writingPrepView" hidden>
        ...
        <p>Kết quả Listening và Reading đang được giữ kín. Khi sẵn sàng, hãy bắt đầu Writing Task 1.</p>
        <ul class="writing-prep-list">
          <li>Tổng thời gian: ${writingConfig.totalMinutes || 30} phút, gồm ${writingConfig.planningMinutes || 10} phút lập dàn ý và 20 phút viết bài.</li>
          <li>Task 1: viết ít nhất 150 từ.</li>
  ...
  elements.readingView.addEventListener('submit', async event => {
    ...
    try {
      if (demoMode === 'exam') {
        ...
        showNotice(serverGradingMode
          ? `Backend test đã chấm Reading: ${state.testGrades.reading.correct}/40 câu đúng. Kết quả sẽ mở sau Writing.`
  ...
  if (writingConfig) {
    ...
    elements.writingView.addEventListener('submit', async event => {
      ...
      try {
        ...
        setStage('result-ready');
        showNotice(automatic
          ? 'Đã hết 30 phút và hệ thống đã thu bài Writing. Bài đang được chấm; bạn có thể tắt trang web và quay lại sau.'
  ...
  function buildDemoPayload(mode) {
    ...
    const reading = mode === 'complete'
      ? serverGradingMode
        ? state.testGrades.reading || makeDemoSection('Reading', 0, 40)
        : makeDemoSection('Reading', 30, 40)
      : null;
    ...
    return {
      ...
      result: {
        testTitle: serverGradingMode
          ? 'Term Test 2 · Khóa 56 · Backend test'
          : 'Term Test 2 · Khóa 56 · Bản minh họa',
        listening,
        reading,
        summary: {
