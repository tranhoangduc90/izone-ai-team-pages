import { createTeacherApi } from './api.js?rev=20260921-class-filter-ops-tabs-v1';
...
const state = { authenticated: false, api: null, timer: null, searchTimer: null, sessionClient: null,
  ...
  visibleColumns: readColumns(), pinnedClasses: readClassList('writing-flow:pinned-classes:v1'),
...
function currentFilters(extra = {}) { return { classCode: $('flow-class').value, teacherName: $('flow-teacher').value,
  ...
  dateFrom: $('flow-date-from').value, dateTo: $('flow-date-to').value, limit: 50, ...extra }; }
...
function actionCell(row) {
  ...
  cell.append(actionButton('Retry', () => void retryPair(row), 'primary'),
    row.skipped_at ? actionButton('Khôi phục', () => void restorePair(row))
...
function openSourceIssueDetail(issue) {
  ...
  source.append(makeText('h3', 'Thông tin nguồn'), makeText('p', `Học viên: ${sourceName(issue)}`),
    ...
    externalLink(issue.file_url, 'Mở Google Docs', ''),
...
function openLegacyDetail(item) {
  ...
  info.append(makeText('h3', 'Thông tin bài'), makeText('p', `Học viên: ${sourceName(item)}`),
    ...
    externalLink(item.file_url, 'Mở Google Docs', ''), externalLink(item.lms_url, 'Mở bài chấm trên LMS', ''));
...
async function loadView() {
  ...
  if (state.activeView === 'classes') { if ($('flow-class').value) { const [stats] = await Promise.all([state.api.writingDailyStats(filters), loadPairs(true)]); state.daily = stats.data.days || []; } else { state.pairs = []; state.daily = []; state.nextCursor = null; } return; }
  ...
  if (state.activeView === 'legacy') { const response = await state.api.writingLegacy(filters.classCode, 0, 50); state.legacy = response.data.records || []; state.pairs = state.legacy.map(legacyRow); state.nextCursor = null; return; }
  await loadPairs(true);
}
